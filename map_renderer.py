...
class TileSprite(pg.sprite.Sprite):
    def __init__(self, group, interface, game_logic, x, y):
        ...
        self.image = pg.Surface((consts.TILE_SIZE, consts.TILE_SIZE)).convert_alpha()
        self.image.fill("#00000000")
        self.wall = interface.tilesheet.subsurface((consts.TILE_SIZE, consts.TILE_SIZE, consts.TILE_SIZE, consts.TILE_SIZE))
        self.wall2 = interface.tilesheet.subsurface((0, consts.TILE_SIZE, consts.TILE_SIZE, consts.TILE_SIZE))
    ...
    def update(self):
        ...
        if not is_explored:
            k *= 0
        if not is_walkable:
            walkable_below = (self.y < consts.MAP_SHAPE[1] - 1) and (self.game_logic.map[self.x, self.y+1] == 1)
            if walkable_below:
